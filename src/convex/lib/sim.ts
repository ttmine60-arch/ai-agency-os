    ...
    const candidates = await ctx.db
      ...
      .filter((q) =>
        q.and(
          ...
          q.gte(q.field("lastContactAt"), 1), // has been contacted
        ),
      )
      .collect();
...
async function buildDemo(
  ...
  const domain = settings.demoDomain ?? "demo.b2kagency.app";
  const slug = slugify(lead.business);
  let built = false;

  if (
    lead.recommendedProduct !== "AI_RECEPTIONIST" &&
    !lead.demoUrl
  ) {
    await ctx.db.insert("demoBuilds", {
      ...
      kind: "website",
      status: "deployed",
      deployUrl: `https://${domain}/${slug}`,
      startedAt: now() - 4 * 60_000,
      finishedAt: now(),
    });
    await ctx.db.patch(lead._id, {
      demoUrl: `https://${domain}/${slug}`,
      updatedAt: now(),
    });
    await logAgent(
      ctx,
      ...
      "PIXEL",
      "ok",
      `Built & deployed personalized website demo for ${lead.business}`,
      { leadId: lead._id },
    );
    built = true;
  }
  ...
  if (
    ...
    await ctx.db.insert("demoBuilds", {
      ...
      kind: "receptionist",
      status: "deployed",
      deployUrl: `https://${domain}/voice/${slug}`,
      startedAt: now() - 3 * 60_000,
      finishedAt: now(),
    });
    await ctx.db.patch(lead._id, {
      receptionistDemoUrl: `https://${domain}/voice/${slug}`,
...
async function sendOffer(
  ...
  // Real delivery in LIVE mode; recorded as sent in DEMO mode.
  // (Convex 1.42+ has no ctx.runAction on mutations — schedule it instead.)
  if (lead.email) {
...
async function orionScore(
  ...
  // VEX follows up with the demo immediately.
  const demo = lead.demoUrl ?? lead.receptionistDemoUrl;
  if (demo) {
...
// ── owner instructions (command chat) ────────────────────────────────────────
...
export async function seedPipeline(
  ...
  for (let i = 0; i < seeds.length; i++) {
    ...
    // research happens for every stage past NEW
    if (stage !== "NEW") {
    ...
    if (stage === "RESEARCHING") {
      ...
      base.status = stage === "MEETING" || stage === "WON" ? stage : stage;
      if (stage !== "QUALIFIED") {
      ...
      if (stage === "DEMO_SENT" || stage === "INTERESTED" || stage === "MEETING" || stage === "WON") {
        base.demoUrl = `https://${settings.demoDomain ?? "demo.b2kagency.app"}/${slugify(biz.business)}`;
  ...
  // seed a few notifications so the owner sees the system was busy
  ...
  const seedLogs: { agent: Doc<"agentLogs">["agent"]; level: "info" | "ok" | "warn"; message: string; leadId?: Id<"leads">; hoursAgo: number }[] = [
    ...
    { agent: "PIXEL", level: "ok", message: "Website demos deployed for qualified leads", hoursAgo: 9 },
